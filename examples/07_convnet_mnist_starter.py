...
with tf.variable_scope('conv1') as scope:
    ...
    reshaped = tf.reshape(X, [-1, 28, 28, 1], name="reshape")

    # create kernel variable of dimension [5, 5, 1, 32]
    # use tf.truncated_normal_initializer()
    
    w1 = tf.get_variable("kernels", [5, 5, 1, 32], dtype=tf.float32, initializer=tf.truncated_normal_initializer())

    # create biases variable of dimension [32]
    # use tf.random_normal_initializer()
    
    b1 = tf.get_variable("biases", [32], dtype=tf.float32, initializer=tf.zeros_initializer())

    # apply tf.nn.conv2d. strides [1, 1, 1, 1], padding is 'SAME'
    
    conv = tf.nn.conv2d(reshaped, w1, strides=[1, 1, 1, 1], padding='SAME')

    # apply relu on the sum of convolution output and biases
    
    conv1 = tf.nn.relu(conv + b1)
...
with tf.variable_scope('fc') as scope:
    ...
    w = tf.get_variable("weight", [input_features, 1024], dtype=tf.float32, initializer=tf.truncated_normal_initializer())
    b = tf.get_variable("bias", [1024], dtype=tf.float32, initializer=tf.zeros_initializer())
...
with tf.variable_scope('softmax_linear') as scope:
    ...
    w = tf.get_variable("weight", [1024, 10], dtype=tf.float32, initializer=tf.truncated_normal_initializer())
    b = tf.get_variable("bias", [10], dtype=tf.float32, initializer=tf.zeros_initializer())
